...
import {
  ...
  BarChart3, Settings, LogOut, X, ChevronLeft, UserCheck,
} from 'lucide-react'
...
const menuItems = [
  ...
  { label: 'Orders', path: '/admin/orders', icon: ShoppingCart, badge: 'pendingOrders' },
...
export default function AdminSidebar({ collapsed, onToggle, mobileOpen, onMobileClose }) {
  const [pendingOrders, setPendingOrders] = useState(getPendingOrdersCount())
            ...
            >
              ...
              >
                ...
                {item.badge === 'pendingOrders' && pendingOrders > 0 && (
                  <span className={`${collapsed ? 'absolute -top-0.5 -right-0.5' : 'ml-auto'} bg-red-500 text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1 shadow-lg shadow-red-500/30`}>
